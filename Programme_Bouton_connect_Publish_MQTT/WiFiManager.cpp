...
void WiFiManager::setupConfigPortal() {
  ...
  DEBUG_WM(F("Configuration du Point d'Accès..."));
  DEBUG_WM(_apName);
  if (_apPassword != NULL) {
    if (strlen(_apPassword) < 8 || strlen(_apPassword) > 63) {
      // fail passphrase to short or long!
      DEBUG_WM(F("Mot de passe du Point d'Accès INCORRECT"));
      _apPassword = NULL;
    }
  ...
  DEBUG_WM(F("Adresse IP Point d'Accès: "));
  ...
  DEBUG_WM(F("Serveur HTTP démarré"));
}
...
void WiFiManager::handleRoot() {
  ...
  page += String(F("<h1>HORIZON-TELECOM</h1>"));
  page += String(F("<h3>Configuration Bouton</h3>"));
...
void WiFiManager::handleWifi(boolean scan) {
  ...
  if (scan) {
    ...
    DEBUG_WM(F("Scan effectué"));
    if (n == 0) {
      DEBUG_WM(F("Réseau non trouvé"));
      page += F("Réseau non trouvé. Scanner pour rechercher");
    } else {
...
void WiFiManager::handleReset() {
  ...
  page += _customHeadElement;
  page += FPSTR(HTTP_HEADER_END);
  page += F("Le système va se redémarrer dans quelques secondes...");
  page += FPSTR(HTTP_END);
...
boolean WiFiManager::captivePortal() {
  if (!isIp(server->hostHeader()) ) {
    DEBUG_WM(F("Requête redirigée vers le portail captif"));
